...
func main() {
	...
	x1Ptr := flag.String("x1", "", "sig1x")
	y1Ptr := flag.String("y1", "", "sig1x")
	x2Ptr := flag.String("x2", "", "sig1x")
	y2Ptr := flag.String("y2", "", "sig1x")
...
//  	SignBLS(curve, *keyPtr, *msgPtr)
	...
	sig := make([]Point, 2)
	sig[0], _ = curve.MakeG1Point(p, true)
	sig[1], _ = curve.MakeG1Point(q, true)
	signature := AggregatePoints(sig)
	fmt.Println(signature.ToAffineCoords()[0])
   	fmt.Println(signature.ToAffineCoords()[1])
